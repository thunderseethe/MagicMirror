...
Module.register("google_calendar",{
	...
	socketNotificationReceived: function(notification, payload) {
		if (notification === "CALENDAR_EVENTS") {
			//console.log('hasCalendarID(%s) = %s', payload.id, this.hasCalendarID(payload.id));
			if (this.hasCalendarID(payload.id)) {
				this.calendarData[payload.id] = payload.events;
				this.loaded = true;
			}
		} else if (notification === "FETCH_ERROR") {
			Log.error("Calendar Error. Could not fetch calendar: " + payload.id);
		} else if (notification === "INCORRECT_URL") {
			Log.error("Calendar Error. Incorrect url: " + payload.id);
		} else {
	...
	getDom: function() {
		...
		for (var e in events) {
			...
			if (this.config.displaySymbol) {
				...
				symbol.className = "fa fa-" + this.symbolForID(event.url);
			...
			if (this.config.displayRepeatingCountTitle) {
				...
				repeatingCountTitle = this.countTitleForID(event.url);
